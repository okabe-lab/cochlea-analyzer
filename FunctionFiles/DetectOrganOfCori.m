...
squareD = squareform(pdist(point4(:,1:2))); %2次元
tpoint3 = point3;
...
Isize = size(originalIm);
im0 = originalIm;
[point3, C, point3_2] = urata_newf66(pointList,originalIm,PIXEL_WIDTH,Z_STEP,0);
if figureon
    urata_newf46(point3)
end

% PCA平面近似し、残差が大きい点を消す、このとき中心方向も推定
...
if 1
    ...
    if 0
        temp3 = point5(:,1:2)-repmat(center_g0,size(point5,1),1);
        rad3 = calc_rad(temp3);
        dist3 = sqrt(temp3(:,1).^2+temp3(:,2).^2);
...
% テンプレートマッチング結果を考慮
if 1
    %point5_2 = point5/coeff;
    point5_2 = point3_2;
    corr = zeros(size(point5_2,1),1);
    ppoint5_2 = point5_2*diag([1/xres 1/xres 1/zres]);
    for i = 1:size(point5_2,1)
    ...
    distth = max(distlist(idx(1:2),:))+25;
    %if sum(f1.*f2)>0
    %    distth = max(distlist(f1.*f2>0))+15;
    %else
    %    distth = max(distlist)+10;
    %end

    point5(dist_2 > distth,:) = [];
    
    if 1
        [distlist rate numlist f1 rate2 f3]
        distth

        figure
        scatter(rad_2,dist_2)
        hold on
        scatter(rad(f>0),dist(f>0),'*')
        plot([min(rad) max(rad)],[distth distth])
        hold off
    end
end

if 0
    % 外有毛細胞を検出し、その外側をノイズ除去
    gain = 3;
    interval = 0.08;
    temp = point5(:,1:2)-repmat(center_g0,size(point5,1),1);
    rad = calc_rad(temp);
    ...
    Z = linkage(Y,'single');
    T = cluster(Z,'cutoff',interval,'criterion','distance'); % 最短距離胞でintervalまでのクラスターを(この値の調整必要）
    ...
    scatter(nrad,ndist,'+')
    hold on
    if sum(tleng > 0.7) > 0 %うまく検出できる場合は使う
        for i = 1:size(candid_group2,1)
            temp_g = candid_group2{i,:};
            
            if tleng(i,1) > 0.7 &&  (max(temp_g(:,2))-min(temp_g(:,2))) < 0.6 && max(temp_g(:,2))>-0.1
                outlines = [outlines; candid_group2{i,:}];
                scatter(candid_group2{i,1}(:,1),candid_group2{i,1}(:,2))
            end
        end
        
        if not(isempty(outlines))
            dist_th = max(outlines(:,2)) + 0.9;
            plot([min(nrad) max(nrad)],[dist_th dist_th])
        end
        hold off
    end   
    stop
...
%% 輝度情報を収集
ppoint5 = (point5/coeff)*diag([1/xres 1/xres 1/zres]);
temppoint = round(ppoint5);
temppoint = sub2ind(Isize,temppoint(:,1),temppoint(:,2),temppoint(:,3));
